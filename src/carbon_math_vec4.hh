...
struct alignas(16) CBN_Vec4 : CBN_Vec4_t {
  ...
  CBN_Vec4 operator+(const CBN_Vec4 &v) const { return carbon_math_vec4_add(*this, v); }
  
  CBN_Vec4 operator+(f32 s) const { return CBN_Vec4(x + s, y + s, z + s, w + s); }
  friend CBN_Vec4 operator+(f32 s, const CBN_Vec4 &v) { return v + s; }
  
  CBN_Vec4 &operator+=(const CBN_Vec4 &v) { return *this = *this + v; }
  ...
  CBN_Vec4 operator*(const CBN_Vec4 &v) const { return carbon_math_vec4_mult(*this, v); }
  
  CBN_Vec4 operator*(f32 s) const { return carbon_math_vec4_scale(*this, s); }
  
  friend CBN_Vec4 operator*(f32 s, const CBN_Vec4 &v) { return v * s; }

  CBN_Vec4 operator/(const CBN_Vec4 &v) const { return carbon_math_vec4_mult(*this, 1/v); }
  
  friend CBN_Vec4 operator/(f32 s, const CBN_Vec4 &v) { return carbon_math_vec4(s/v.x, s/v.y, s/v.z, s/v.w); }

  void Clamp(const CBN_Vec4 &min, const CBN_Vec4 &max) { *this = carbon_math_vec4_clamp(*this, min, max); }
  ...
  CBN_Vec4 Lerp(const CBN_Vec4 &v, f32 t) const { return carbon_math_vec4_lerp(*this, v, t); }
