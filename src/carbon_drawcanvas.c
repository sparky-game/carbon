...
void carbon_drawcanvas_fill(CBN_DrawCanvas *dc, u32 color, bool blend) {
  ...
  if (blend) {
    ...
    for (usz i = 0; i < n; ++i) dc->pixels[i] = color;
  }
}
...
void carbon_drawcanvas_sprite(CBN_DrawCanvas *dc, const CBN_Sprite *s, CBN_Vec2 position, CBN_Vec2 scale, u32 tint) {
  ...
  {// Clipping
    f32 ox = xywh.x, oy = xywh.y;
    xywh.x = carbon_math_floor(ox), xywh.y = carbon_math_floor(oy);
    xywh.w = carbon_math_ceil(ox + xywh.w) - xywh.x;
    xywh.w = carbon_math_min(xywh.w, r_dc.w - xywh.x);
    xywh.h = carbon_math_ceil(oy + xywh.h) - xywh.y;
...
void carbon_drawcanvas_mesh(CBN_DrawCanvas *dc, const CBN_Camera *c, const CBN_Mesh *m, CBN_Transform t, u32 tint) {
  ...
  for (usz f = 0; f < m->metadata.faces_count; ++f) {
    ...
    carbon_drawcanvas__poly_triangulation(dc, pvs, pvs_count, tint);
  }
}
...
void carbon_drawcanvas_mesh_with_texture(CBN_DrawCanvas *dc, const CBN_Camera *c, const CBN_Mesh *m, CBN_Transform t, const CBN_Sprite *s, u32 tint) {
  ...
  for (usz f = 0; f < m->metadata.faces_count; ++f) {
    ...
    Vertex3D pvs[4];
    usz pvs_count = carbon_drawcanvas__near_plane_clipping(v1, v2, v3, pvs);
    carbon_drawcanvas__poly_triangulation_with_texture(dc, pvs, pvs_count, s, tint);
  }
}
