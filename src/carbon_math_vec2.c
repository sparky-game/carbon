...
bool carbon_math_vec2_barycentric(CBN_Vec2 v1, CBN_Vec2 v2, CBN_Vec2 v3, CBN_Vec2 p, CBN_Vec3 *u) {
  ...
  if (carbon_math_abs(det) < CARBON_EPS) return false;  // triangle{v1,v2,v3} is degenerate
  ...
  if (u1 < -CARBON_EPS || u2 < -CARBON_EPS || u3 < -CARBON_EPS) return false;  // p is outside the triangle{v1,v2,v3}
  return true;
}
