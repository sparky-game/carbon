...
CBNINL u32 *carbon_image_8bit_to_32bit(const u8 *data, usz wh) {
  u32 *pxs = carbon_memory_alloc(wh * 4);
  for (usz i = 0; i < wh; ++i) {
    u32 r = data[i * 4 + 0];
    u32 g = data[i * 4 + 1];
    u32 b = data[i * 4 + 2];
    u32 a = data[i * 4 + 3];
    u32 px = (r << 24) | (g << 16) | (b << 8) | a;
    pxs[i] = px;
  }
  return pxs;
}
...
void carbon_image_destroy(CBN_Image *img) {
  if (img->owned) carbon_memory_free(img->pixels);
