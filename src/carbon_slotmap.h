...
#define carbon_slotmap_foreach(T, sm) if ((sm).size) for (struct { usz i; T var; } it = {0, carbon_list_at(T, (sm).data, 0)}; it.i < ((sm).data).size; ++it.i, it.i < ((sm).data).size ? it.var = carbon_list_at(T, (sm).data, it.i) : it.var)
