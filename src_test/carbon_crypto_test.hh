...
TEST(chacha20) {
  // Using official test vectors: <https://www.ietf.org/archive/id/draft-strombergson-chacha-test-vectors-00.txt>.
#define TC(...) {                                         \
  const u8 expected[64*2] {__VA_ARGS__};                  \
  for (usz i = 0; i < CARBON_ARRAY_LEN(expected); ++i) {  \
  carbon_should_be(expected[i], cipher[i]);               \
  }                                                       \
  }
  ...
  {// TC1: All zero key and IV.
    ...
    const u8 plain[64*2] {};
    u8 cipher[64*2] {};
    cbn::crypto::ChaCha20(key, iv, cbn::Span<u8>(plain, CARBON_ARRAY_LEN(plain)), cipher);
    TC(0x76, 0xb8, 0xe0, 0xad, 0xa0, 0xf1, 0x3d, 0x90,  // Keystream block 1
       ...
       0x9f, 0x07, 0xe7, 0xbe, 0x55, 0x51, 0x38, 0x7a,  // Keystream block 2
  ...
  {// TC8: Random key and IV.
    ...
    u8 cipher[64*2] {};
    cbn::crypto::ChaCha20(key, iv, cbn::Span<u8>(plain, CARBON_ARRAY_LEN(plain)), cipher);
    TC(0xf6, 0x3a, 0x89, 0xb7, 0x5c, 0x22, 0x71, 0xf9,  // Keystream block 1
       ...
       0xe5, 0xfb, 0xc3, 0x4e, 0x60, 0xa1, 0xd9, 0xa9,  // Keystream block 2
