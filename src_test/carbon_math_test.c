...
TEST(pow) {
  ...
  carbon_should_be_f(13.797444, carbon_math_pow(1.4, 7.8));
  return CARBON_OK;
}
...
TEST(vec2_add) {
  CBN_Vec2 u = carbon_math_vec2(1, 2);
  CBN_Vec2 v = carbon_math_vec2(3, 4);
...
TEST(rect_contains) {
  CBN_Rect r = {0, 0, 10, 5};
  CBN_Vec2 p1 = carbon_math_vec2(8, 3);
  CBN_Vec2 p2 = carbon_math_vec2(3, 8);
