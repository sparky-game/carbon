...
~ Every ~Carbon~ user ever
#+end_quote

~Carbon~ is the ultimate *high-level* library for C/C++ written in *pure C (C99)*: it's tiny, easy to use and it can be employed to make any kind of application, from CLI utilities to graphics-heavy games and GUI programs. @@html:<br>@@
It even can be used to test itself, as well as any other piece of C code; thus, working as a full-fledged testing suite. Cool, isn't it? @@html:<br>@@
