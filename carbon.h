...
#define carbon_circularqueue_at(T, cq, i) (CARBON_ASSERT(0 <= (i32) (i) && (i) < (cq).size && "CircularQueue index out of bounds"), CARBON_ASSERT(sizeof(T) == (cq).stride && "CircularQueue type doesn't match"), carbon_circularqueue_at_raw(T, cq, i))
...
const typename CBN_CircularQueue_t<T>::value_type &CBN_CircularQueue_t<T>::operator[](usz idx) const {
  CARBON_ASSERT(0 <= idx && idx < size && "CircularQueue index out of bounds");
...
#include "src/carbon_circularqueue.c"
...
#include "src/carbon_drawcanvas.cc"
